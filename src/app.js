...
function escapeHtml(s) {
  // Tolère null/undefined/nombre : appelée sur des données de tâche qui peuvent
  // venir d'un JSON importé.
...
// RENDER
// ═══════════════════════════════════════════════════
// `persist` à false : rendu seul, sans écrire en localStorage. Réservé aux
// rendus intermédiaires d'un glisser-déposer (des dizaines par seconde) — la
// sauvegarde a lieu une fois, au relâchement.
...
function buildTooltip(task) {
  ...
  let html = `<strong style="color:var(--text)">${escapeHtml(task.name)}</strong><br>`;
  ...
  if (task.description) html += `<br><span style="color:var(--text3);font-size:11px">${escapeHtml(task.description.substring(0,100))}${task.description.length > 100 ? '…' : ''}</span>`;
  return html;
}
...
function makeDraggable(bar, task, range, pxDay) {
  bar.addEventListener('mousedown', (e) => {
    ...
    // qu'une fois par frame, et sans écrire en localStorage (sérialiser tout le
    // projet à chaque pixel saccade sur un gros planning).
    let frame = null;
    let moved = false;
    const scheduleRender = () => {
      if (frame !== null) return;
      frame = requestAnimationFrame(() => { frame = null; render(false); });
    };
    ...
    const onUp = () => {
      ...
      // Rendu final, celui-là persisté : c'est la seule écriture du glissement.
      if (moved) render();
    };
...
function makeNestOption(targetId, label, isCurrent) {
  ...
  if (isCurrent) {
    opt.innerHTML = `<span style="font-size:10px;color:var(--accent)">✓</span><span style="overflow:hidden;text-overflow:ellipsis">${escapeHtml(label)}</span>`;
  } else {
...
// Un projet peut venir d'un fichier JSON quelconque ou d'un localStorage écrit
// par une version antérieure : rien n'y est digne de confiance. Plutôt que de
// refuser le fichier, on le répare (valeurs par défaut, liens cassés remis à la
// racine, cycles rompus) et on rend compte des corrections à l'utilisateur —
// il ne doit jamais perdre son projet à cause d'un champ abîmé.
//
// Fonction pure (aucun accès au DOM) → testée dans tests/import.test.js.

const VALID_TASK_TYPES = new Set(['standard', 'milestone', 'summary']);

function isValidDateStr(s) {
  if (typeof s !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(s)) return false;
  const [y, m, d] = s.split('-').map(Number);
  const dt = new Date(y, m - 1, d);
  // Rejette les dates syntaxiquement correctes mais inexistantes (2026-02-31).
  return dt.getFullYear() === y && dt.getMonth() === m - 1 && dt.getDate() === d;
}

function sanitizeProject(data) {
  if (!data || typeof data !== 'object' || !Array.isArray(data.tasks)) {
    throw new Error('format invalide (aucune liste de tâches)');
  }
  ...
  // 1) Nettoyage tâche par tâche
  const clean = [];
  const seenIds = new Set();
  data.tasks.forEach((raw, i) => {
    if (!raw || typeof raw !== 'object') {
      warnings.push(`Entrée ${i + 1} ignorée (ce n'est pas une tâche).`);
      return;
    }
    if (!Number.isInteger(raw.id)) {
      warnings.push(`Tâche ${label(raw, i)} ignorée (identifiant manquant ou invalide).`);
      return;
    }
    if (seenIds.has(raw.id)) {
      warnings.push(`Tâche ${label(raw, i)} ignorée (identifiant ${raw.id} en double).`);
      return;
    }
    seenIds.add(raw.id);

    let startDate = isValidDateStr(raw.startDate) ? raw.startDate : null;
    let endDate = isValidDateStr(raw.endDate) ? raw.endDate : null;
    if (!startDate && !endDate) {
      startDate = endDate = fallbackDate;
      warnings.push(`Tâche ${label(raw, i)} : dates illisibles, remplacées par aujourd'hui.`);
    } else if (!startDate) {
      startDate = endDate;
      warnings.push(`Tâche ${label(raw, i)} : date de début illisible, alignée sur la fin.`);
    } else if (!endDate) {
      endDate = startDate;
      warnings.push(`Tâche ${label(raw, i)} : date de fin illisible, alignée sur le début.`);
    }
    if (endDate < startDate) {
      endDate = startDate;
      warnings.push(`Tâche ${label(raw, i)} : fin antérieure au début, corrigée.`);
    }

    let type = raw.type;
    if (!VALID_TASK_TYPES.has(type)) {
      if (type !== undefined && type !== null) {
        warnings.push(`Tâche ${label(raw, i)} : type inconnu, ramenée à une tâche standard.`);
      }
      type = 'standard';
    }

    const name = typeof raw.name === 'string' && raw.name.trim() ? raw.name : 'Sans nom';
    if (name === 'Sans nom') warnings.push(`Tâche n° ${i + 1} : nom manquant.`);
    ...
    clean.push({
      ...
      order: Number.isFinite(raw.order) ? raw.order : clean.length,
      collapsed: raw.collapsed === true,
    });
  });

  // 2) Liens de hiérarchie : parent inexistant ou auto-référence → racine
  const byId = new Map(clean.map(t => [t.id, t]));
  for (const t of clean) {
    if (t.parentId === null) continue;
    if (t.parentId === t.id || !byId.has(t.parentId)) {
      warnings.push(`Tâche « ${t.name} » : parent introuvable, replacée à la racine.`);
      t.parentId = null;
    }
  }

  // 3) Cycles : on remonte les ancêtres et on coupe le lien qui referme la boucle
  for (const t of clean) {
    ...
    while (cur.parentId !== null) {
      ...
      if (seen.has(parent.id)) {
        warnings.push(`Hiérarchie circulaire : « ${cur.name} » a été replacée à la racine.`);
        cur.parentId = null;
        break;
      }
      seen.add(parent.id);
  ...
  const maxId = clean.reduce((m, t) => Math.max(m, t.id), 0);
  const nextId = Number.isInteger(data.nextId) && data.nextId > maxId ? data.nextId : maxId + 1;
  const name = typeof data.name === 'string' && data.name.trim() ? data.name : null;

  return { tasks: clean, nextId, name, warnings };
}

// Message récapitulatif des corrections, tronqué pour rester lisible.
function formatWarnings(warnings) {
  const shown = warnings.slice(0, 8);
  const rest = warnings.length - shown.length;
  return `${warnings.length} anomalie(s) corrigée(s) dans le projet :\n\n• ${shown.join('\n• ')}`
    + (rest > 0 ? `\n\n… et ${rest} autre(s).` : '');
}
...
function onFileLoad(e) {
  ...
  reader.onload = (ev) => {
    try {
      const clean = sanitizeProject(JSON.parse(ev.target.result));
      tasks = clean.tasks;
      nextId = clean.nextId;
      document.getElementById('project-name').value = clean.name || 'Projet chargé';
      selectedTaskId = null;
      render();
      showSaveIndicator('Projet chargé ✓');
      if (clean.warnings.length) alert(formatWarnings(clean.warnings));
    } catch(err) {
...
function loadFromLocalStorage() {
  ...
  if (!raw) return false;
  try {
    // Même passage par sanitizeProject() que l'import de fichier : le
    // localStorage peut contenir un projet écrit par une version antérieure.
    // Silencieux ici (pas d'alerte au démarrage) — les corrections seront
    // simplement persistées au premier render().
    const clean = sanitizeProject(JSON.parse(raw));
    tasks = clean.tasks;
    nextId = clean.nextId;
    document.getElementById('project-name').value = clean.name || 'Mon Projet';
    return true;
  } catch { return false; }
}
